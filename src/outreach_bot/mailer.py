...
def render_template(template: str, company: Company, settings: Settings) -> str:
    values = {"company_name": company.name, "website_url": company.website_url, "sender_name": settings.sender_name, "sender_company": settings.sender_company, "sender_email": settings.sender_email, "sender_phone": settings.sender_phone, "opt_out_email": settings.opt_out_email}
...
def build_email(company: Company, campaign: Campaign, settings: Settings) -> EmailMessage:
    ...
    footer = f"\n\n---\n送信者: {settings.sender_company} {settings.sender_name}\n今後のご案内が不要な場合は {settings.opt_out_email} へご連絡ください。"
    message.set_content(body + footer)
    return message


def send_email(company: Company, campaign: Campaign, settings: Settings, *, dry_run: bool) -> str:
