...
async def run_campaign(session: Session, campaign_id: int, settings: Settings, *, dry_run: bool = True) -> dict[str, int]:
        ...
        for channel in channels:
            ...
            try:
                if channel == "email":
                    ...
                    result = await submit_contact_form(company, subject, body, settings, dry_run=dry_run)
            ...
            attempt = OutreachAttempt(company_id=company.id or 0, campaign_id=campaign.id or 0, channel=channel, destination=destination, status=status, reason=reason, idempotency_key=key)
            session.add(attempt)
            session.commit()
...
def export_companies(session: Session, path: Path) -> int:
    ...
    fields = ["name", "canonical_domain", "website_url", "source_url", "industry", "prefecture", "contact_email", "contact_phone", "contact_form_url", "consent_basis", "no_solicitation", "last_seen_at"]
